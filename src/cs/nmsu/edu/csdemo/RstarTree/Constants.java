...
//                System.out.println(mbr1[2 * i + 1] +","+ mbr2[2 * i] );
    ...
    public static float objectDIST(PPoint point1, PPoint point2) {
        ...
        for (i = 0; i < point1.dimension; i++) {
            sum += Math.pow(point1.data[i] - point2.data[i], 2);
        }
    ...
    /**
     * computes the MINDIST between 1 pt and 1 MBR (see [Rous95]) the MINDIST
     * ensures that the nearest neighbor from this pt to a rect in this MBR is
    ...
    public static float MINDIST(Object pt, float bounces[]) {
        ...
        for (i = 0; i < point.dimension; i++) {
            ...
            sum += Math.pow(point.data[i] - r, 2);
        }
        return (sum);
    }

    /**
     * computes the MAXDIST between 1 pt and 1 MBR. It is defined as the maximum
     * distance of a MBR vertex against the specified point Used as an upper
     * bound of the furthest rectangle inside an MBR from a specific point
     */
    public static float MAXDIST(Object pt, float bounces[]) {
        ...
        for (i = 0; i < point.dimension; i++) {
            ...
            maxdiff = max(Math.abs(point.data[i] - bounces[2 * i]),
                    Math.abs(point.data[i] - bounces[2 * i + 1]));
            sum += Math.pow(maxdiff, 2);
        }
        return (sum);
    }

    /**
     * computes the MINMAXDIST between 1 pt and 1 MBR (see [Rous95]) the
     * MINMAXDIST ensures that there is at least 1 object in the MBR that is at
     * most MINMAXDIST far away of the point
     */
    public static float MINMAXDIST(PPoint point, float bounces[]) {
        ...
        for (k = 0; k < point.dimension; k++) {
            ...
            sum += S - Math.pow(point.data[k] - rMi, 2);
