package cs.nmsu.edu.csdemo.RstarTree;
 ...
 * +-----------+-------------+---------------+---------------+--------------+------+
 * | dimension | num_of_data | num_of_dnodes | num_of_inodes | root_is_data |
 * root |
 * +-----------+-------------+---------------+---------------+--------------+------+
 *
 */
public final class RTree {
 ...
 /*
     ...
     */
    public RTree(String fname, int _b_length, int cache_size, int _dimension) // neuen R-Baum konstruieren
    {
        ...
        // This is actually nonsense here, but this sets the variable user_header correctly, so that when writing the header the caller has no problem
        try {
            read_header(header);
        } catch (Exception e) {
        }
        ...
        //let root_ptr point to a empty data node 
        root_ptr = new RTDataNode(this);
//        System.out.println(root_ptr.block);
        root = root_ptr.block;
//        System.out.println(root+" "+root_ptr.num_entries);
    }

    /**
     * Construct(read) an R*-tree from disk
     */
    public RTree(String fname, int cache_size) // bereits vorhandenen R-Baum laden
    {
        node_weight = new float[20];
...
//                System.out.println("root is not data");
     ...
     * root node. If the root overflows, it has to be split into 2 nodes and a
     * new root will be introduced, to hold the split nodes.
     */
...
//        System.out.println("root_ptr is null ? "+ (root_ptr == null) );
//        System.out.println("this.root_is_data?" + this.root_is_data);
        load_root();
//
        /*
         ...
         */
//        System.out.println("level:" + root_ptr.level);
...
//            System.out.println(d_cand.getPlaceId());
            if (d_cand != null) {
...
//                System.out.println(root_ptr.getClass().getName()+" "+split_root);
            } else {
...
//                System.out.println(sn[0].getClass().getName());
    ...
    /**
     * Return the ith data element in the tree. --> Propagate to the root node.
     */
    Data get(int i) {
        Data d;
...
//        System.out.println(root_ptr.getClass().getName());
     ...
     */
    /**
     * Return to Nearest the object in the tree that is nearest to parameter
     * point. --> Propagate to the root node.
     */
    ...
    /**
     * Return to res the objects in the tree that are nearest to parameter
     * point. --> Propagate to the root node.
     */
    ...
    /**
     * Return to res the k objects in the tree that are nearest to parameter
     * point. --> Propagate to the root node.
     */
    ...
    /**
     * Return to res the objects in the tree that intersect with the parameter
     * point. --> Propagate to the root node.
     */
    ...
    /**
     * Return to res the objects in the tree that intersect with the parameter
     * circle. --> Propagate to the root node.
     */
    ...
    /**
     * Return to res the objects in the tree that intersect with the parameter
     * ring, defined by the circles provided that radius1<radius2.
     * --> Propagate to the root node.
     */
    ...
    /**
     * Return to res the objects in the tree that intersect with the parameter
     * mbr. --> Propagate to the root node.
     */
    ...
    /**
     * Store to node_weight[] how many of the directory pages starting from the
     * root level can be stored in a cache with the paremeter cache size. Each
     * entry in node_weight stores the fraction of nodes at the specific level
     * that will not fit in the cache. For instance,
     * node_weight[root_ptr.level=10]=0, node_weight[9]=0, node_weight[8]=0,
     * node_weight[7]=0.4, node_weight[6]=1,... means that only the entries of
     * the upper 3 levels can fully fit in cache, and also a portion of the
     * nodes at the 7th level.
     */
    void set_node_weight(int cache_size) {
        ...
        for (i = root_ptr.level; i > 0; i--) {
            if (c) // cache is not fully filled
            {
                if (rest - nodes_tab[i] > 0) // there is enough cache for this level
                {
                    rest -= nodes_tab[i];
